...
%% Supplementary Plot for Frequency and Power Coefficients of Variation
figure(2);
varTit = {'Peak Frequency (Hz)', 'Power COV', 'Frequency COV'};
varInd = [1, 5, 6];
subind = 12:23;

for ip = 1:3
    subplot(2, 3, ip + 3);
    V = violinplot(squeeze(blockRow(varInd(ip), :, subind))', {'Rest', 'Posture'}, 'Width', 0.2);
    X = [];
    Y = [];
    for i = 1:2
        X(:, i) = V(i).ScatterPlot.XData;
