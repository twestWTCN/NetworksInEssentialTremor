...
%% Venn Diagram
% % data = [sum(subSig(:,1)) sum(all(subSig(:,[1 2]),2)) sum(subSig(:,2))  sum(all(subSig(:,[2 3]),2)) sum(subSig(:,3)) sum(all(subSig(:,[1 3]),2)) sum(all(subSig(:,[1 2 3]),2))];
% % % tit('Cer','Cer&dlPFC','dlPFC','dlPFC&PPC','PPC','PPC&Cereb','PPC&Cereb&dlPFC')
% % vennX(data,.01)
% % axis equal
