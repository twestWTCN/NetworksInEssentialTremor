...
function plotEpochingOutcomes(R, ftdata, coder, levels, SMRtrialdef)
% PLOTEPOCHINGOUTCOMES visualizes epoching outcomes by plotting the data alongside the trial definitions.

% Plot coder signal
plot(ftdata.time{1}, coder, 'k--');
hold on;

% Plot trial data
yyaxis right;
plot(ftdata.time{1}, ftdata.trial{1}(end-2:end, :), 'r');

yyaxis left;

% Plot each trial definition
for prt = 1:size(SMRtrialdef, 1)
    trialdef = SMRtrialdef{prt, 2} ./ ftdata.fsample;
    for trl = 1:size(trialdef, 2)
        p = plot(trialdef(:, trl), repmat(levels(prt), 1, 2));
        p.Color = R.plot.cmap.lplots(prt, :);
        p.LineWidth = 3;
    end
    pexamp(prt) = p;
end

legend(pexamp, R.epoch.names);

% Set y-axis limits if levels are small
if max(levels) < 0.1
    ylim([0 0.05]);
end
